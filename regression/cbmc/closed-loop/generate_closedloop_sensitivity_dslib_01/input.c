...
int main(){
   ...
   double c_num[100] = controller.b;
   double c_den[100] = controller.a;

   /* Quantizing Control Coefficients */
   fxp32_t c_num_fxp[controller.b_size];
   fxp_double_to_fxp_array(c_num, c_num_fxp, controller.b_size);
   fxp32_t c_den_fxp[controller.a_size];
   ...
   double b_ans[100];
   int b_ans_size = controller.a_size + plant.a_size - 1;
   double a_ans[100];
   int a_ans_size = controller.a_size + plant.a_size - 1;

   ft_closedloop_sensitivity(c_num_qtz, controller.b_size, c_den_qtz, controller.a_size, p_num, p_num_size, p_den, p_den_size, b_ans, b_ans_size, a_ans, a_ans_size);
   ...
   assert(b_ans[0] == 1);
   assert(b_ans[1] == 0.3000030517578125);
   assert(b_ans[2] == -0.529985961969942);
   assert(b_ans[3] == -0.0869872132316235);
   assert(b_ans[4] == 0.0555977844633165);
   assert(b_ans[5] == -0.004202099516988);
   assert(b_ans[6] == 0);
