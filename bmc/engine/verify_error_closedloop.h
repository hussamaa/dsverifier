...
int verify_error_closedloop(void)
{
	...
	ft_closedloop_series(c_num_qtz, c_num_size, c_den_qtz, c_den_size, p_num,
			p_num_size, p_den, p_den_size, ans_num_qtz, ans_num_size,
			ans_den_qtz, ans_den_size);
	ft_closedloop_series(c_num, c_num_size, c_den, c_den_size, p_num,
			p_num_size, p_den, p_den_size, ans_num_double, ans_num_size,
			ans_den_double, ans_den_size);
	...
	for (i = 0; i < X_SIZE_VALUE; ++i)
	{
		...
		y_double[i] = double_direct_form_1(yaux_double, xaux_double,
		...
		y_double[i] = double_direct_form_2(waux_double, x_double[i],
				ans_den_double, ans_num_double, ans_den_size, ans_num_size);
#endif
		...
		/* realization with controller quantized */
		y_qtz[i] = double_transposed_direct_form_2(waux_qtz, x_qtz[i],
				ans_den_qtz, ans_num_qtz, ans_den_size, ans_num_size);
		/* realization with controller non quantized */
