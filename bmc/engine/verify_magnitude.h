 ...
 */
 ...
 int verify_magnitude(void) {
	...
	resp_mag(_b, ds.b_size, _a, ds.a_size, res, N);



	/* generates magnitude response, placing the result in the "res" array*/

	if (prop.type == LOWPASS) { //lowpass
		for (i = 0, w = 0; (w <= 1.0); ++i, w += w_incr) {
			if (w <= prop.wp) {
				__DSVerifier_assert(res[i] >= prop.Ap, "|----------------Passband Failure-------------|");
			} else if (w == prop.wc) {
				__DSVerifier_assert(res[i] <= prop.Ac, "|-------------Cutoff Frequency Failure--------|");
			} else if ((w >= prop.wr) && (w <= 1)) {
				__DSVerifier_assert(res[i] <= prop.Ar, "|----------------Stopband Failure-------------|");
			}
		}
	} else if (prop.type == HIGHPASS) { //highpass
		for (i = 0, w = 0; (w <= 1.0); ++i, w += w_incr) {
			if (w <= prop.wr) {
				__DSVerifier_assert(res[i] <= prop.Ar, "|----------------Stopband Failure-------------|");
			} else if (w == prop.wc) {
		...
		__DSVerifier_assert(0, "|----------------Filter Type Not Supported-------------|");
