...
void double_add_matrix(unsigned int lines, unsigned int columns,
		double m1[LIMIT][LIMIT], double m2[LIMIT][LIMIT],
		double result[LIMIT][LIMIT])
{
	unsigned int i, j;
	for (i = 0; i < lines; i++)
	{
...
void fxp_matrix_multiplication(unsigned int i1, unsigned int j1,
		unsigned int i2, unsigned int j2, fxp_t m1[LIMIT][LIMIT],
		fxp_t m2[LIMIT][LIMIT], fxp_t m3[LIMIT][LIMIT])
{
	unsigned int i, j, k;
	if (j1 == i2)
	{ //Checking if the multiplication is possible
...
void fxp_exp_matrix(unsigned int lines, unsigned int columns,
		fxp_t m1[LIMIT][LIMIT], unsigned int expNumber,
		fxp_t result[LIMIT][LIMIT])
{
	...
	for (l = 1; l < expNumber; l++)
	{
		...
		for (i = 0; i < lines; i++)
		{
			for (j = 0; j < columns; j++)
			{
				for (k = 0; k < columns; k++)
				{
					result[i][j] = fxp_add(result[i][j],
							fxp_mult(m2[i][k], m1[k][j]));
				}
			}
		}
	}
...
void double_exp_matrix(unsigned int lines, unsigned int columns,
		double m1[LIMIT][LIMIT], unsigned int expNumber,
		double result[LIMIT][LIMIT])
{
...
void fxp_add_matrix(unsigned int lines, unsigned int columns,
		fxp_t m1[LIMIT][LIMIT], fxp_t m2[LIMIT][LIMIT],
		fxp_t result[LIMIT][LIMIT])
{
	unsigned int i, j;
	for (i = 0; i < lines; i++)
...
/* prints a matrix */
void print_matrix(double matrix[LIMIT][LIMIT], unsigned int lines,
