   ...
   for CURRENT_IMPLEMENTATION in $(seq 1 $TOTAL_OF_IMPLEMENTATIONS); do
       ...
       for CURRENT_REALIZATION in ${REALIZATIONS[@]}; do
           ...
           for CURRENT_PROPERTY in ${PROPERTIES[@]}; do
              ...
              OUT_FILE="$OUTPUT_LOGS_DIRECTORY/$CURRENT_PROPERTY/result-c$CURRENT_DS_ID-impl$CURRENT_IMPLEMENTATION-$CURRENT_REALIZATION.out";   

              # do the verification
              INITIAL_EXECUTION_TIMESTAMP=$(date +%s)
              { time ./dsverifier/esbmc -DSVERIFIER --boolector $BENCHMARKS_LIBRARY -DDS_ID=$CURRENT_DS_ID -DIMPLEMENTATION_ID=$CURRENT_IMPLEMENTATION -DREALIZATION=$CURRENT_REALIZATION -DPROPERTY=$CURRENT_PROPERTY -DX_SIZE=$X_SIZE --timeout $TIMEOUT > $OUT_FILE; } 2>> $OUT_FILE ;
