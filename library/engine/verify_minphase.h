...
#include <assert.h>
...
#ifndef _VERIFY_MINPHASE_H

	#define _VERIFY_MINPHASE_H	1
	...
	int verify_minphase(void){
		/* check the realization */
			...
			assert(esbmc_check_stability_double(_b, ds.b_size));
			...
			assert(__DSVERIFIER_check_delta_stability(db, DEADLINE, impl.int_bits, impl.frac_bits));
			exit(1);
			...
			/* check stability using delta domain (intrinsic function) */
			assert(__DSVERIFIER_check_delta_stability_cascade(db_cascade, ds.sample_time, impl.int_bits, impl.frac_bits));
			exit(1);
		#endif
		return 0;
	}
