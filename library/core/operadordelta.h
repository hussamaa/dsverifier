#include <assert.h>
...
#ifndef _OPERADORDELTA_H

	#define _OPERADORDELTA_H	1
	#ifdef __cplusplus
		extern "C" {
		}
	#endif
	...
	int esbmc_check_stability_double_closedloop(double a[], int n, double plant_num[], int p_num_size, double plant_den[], int p_den_size){
	   ...
	   /* to put current values in stability counterexample
		* look for current_stability (use: --no-slice) */
	   double current_plant_num[p_num_size];
	   for (i=0; i < p_num_size; i++){
		   current_plant_num[i] = plant_num[i];
	   }
	   double current_plant_den[p_den_size];
   	   for (i=0; i < p_den_size; i++){
   		   current_plant_den[i] = plant_den[i];
   	   }
	   double current_stability[n];
	   for (i=0; i < n; i++){
		   current_stability[i] = a[i];
