...
void apply_householder_qr(double **mat, unsigned int rows, 
		       unsigned int cols, double *house, unsigned int row_pos,
		       unsigned int *p);
...
/* Solve the least squares problem, sol = mat\rhs . */
void qr(double **mat, unsigned int rows, unsigned int cols);
...
void apply_householder_qr(double **mat, unsigned int rows, unsigned int cols, double *house, unsigned int row_pos, unsigned int *p)
{
  int i, j, k, n;
  double sum;
  double **hhmat;
...
void qr(double **mat, unsigned int rows, unsigned int cols){
  ...
  for(i = 0; i < cols; i++){
      ...
      householder(mat, rows, cols, i, p[i], v);
      apply_householder_qr(mat, rows, cols, v, i, p);
   }

   printf("A': \n");
   display_mat(mat, rows,cols, NULL);
